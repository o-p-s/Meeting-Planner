/*** DEPENDENCIES */
const mongoose = require('mongoose'),shortid = require('shortid'),time = require('./../libs/timeLib');
/**CONFIGS */
const mailerConfig=require('./../../config/mailerConfig');
/** LIBRARIES */
const response = require('./../libs/responseLib'), logger = require('./../libs/loggerLib'),
validateInput = require('../libs/paramsValidationLib'),check = require('../libs/checkLib'),
passwordLib = require('./../libs/passwordLib'),token=require('./../libs/tokenLib'),
mailerLib= require('./../libs/mailerLib');
/** MODELS */
const UserModel=mongoose.model('User'), AuthModel=mongoose.model('Auth'),
signUpTokenModel=mongoose.model('signUpToken'),passwordResetTokenModel=mongoose.model('passwordResetToken');

const url="http://meeting-planner.opsaini.com";

let email = mailerConfig.auth.email;
        ...
        return new Promise((resolve,reject)=>{
            if(req.body.email && req.body.password){
                if(!validateInput.Email(req.body.email)){
                    ...
                    .select('-mobileNumber -reset_password_token -reset_password_expires')
                    .lean()
